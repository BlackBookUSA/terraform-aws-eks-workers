...
resource "aws_security_group_rule" "sister_cluster" {
  ...
  protocol                 = "-1"
  security_group_id        = join("", aws_security_group.default.*.id)
  source_security_group_id = var.sister_cluster_security_group_id
  type                     = "ingress"
}
...
module "autoscale_group" {
  source = "git::https://github.com/blackbookusa/terraform-aws-ec2-autoscale-group.git?ref=tags/0.1.12"
